...
class PurePursuitFollower2D:
    def __init__(
        ...
        max_ax: float = 0.5,  
        L_min: float = 0.10,   # 最小前瞻（m）
        L_max: float = 0.40,   # 最大前瞻（m）
        k_v: float = 0.60,     # 速度对前瞻的放大系数
        k_ang: float = 1.00,   # 角度对前瞻的抑制强度
        k_goal: float = 0.80   # 接近终点时的压缩强度   
    ):
        self.path = path
    ...
    def find_lookahead_point(self, current_pos: Tuple[float, float], lookahead: Optional[float] = None) -> Optional[Tuple[float, float]]:
        L = self.lookahead_distance if lookahead is None else lookahead
    ...
    def compute_control(
        ...
        # ===== 1. 查找前瞻点 =====
        angle_diff_guess = 0.0
        L_dyn = self._dynamic_lookahead(current_pos, angle_diff_guess, self.last_vx)

        # ===== 2. 用动态 L 查找前瞻点 =====
        target = self.find_lookahead_point(current_pos, lookahead=L_dyn)
        if target is None:
            self.last_vx = self.last_vyaw = 0.0
            self.last_done = True
            return 0.0, 0.0, True

        # 现在有了目标点，精确计算角度误差，再细调一次 L（可选）
